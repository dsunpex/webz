        ...
        switch ($this->cur_state)
        {
            case static::BUFFERING:
                ...
                if ($end_response_headers === false)
                {
					...
					if(!isset($ini["error_page_502"])){
					$response = $this->server->text_response(502, "<center><h1>503</h1><p>Invalid Response from CGI process</p><hr><sub>WebZHTTPServer/1.0</sub></center>");
					}
					$response = $this->text_response(502, $ini["error_page_502"]);
                }
                else
                {                
