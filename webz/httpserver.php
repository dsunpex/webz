...
class HTTPServer
{
    ...
    public $server_id = 'WebzHTTPServer/1.0';   // identifier string to use in 'Server' header of HTTP response
    ...
    function route_request($request)
    {
		...
		if(!isset($ini["error_page_500"])){
			return $this->text_response(500, "<center><h1>500</h1><p>Not implemented</p><hr><sub>WebZHTTPServer/1.0</sub></center>");
		}
		return $this->text_response(500, $ini["error_page_500"]);
    }    
    ...
    function read_request_complete($request)
    {
        ...
        {
			$ini = parse_ini_file("webz.ini");
			if(!isset($ini["error_page_403"])){
			$response = $this->text_response(403, "<center><h1>403</h1><p>Bad URI</p><hr><sub>WebZHTTPServer/1.0</sub></center>"); 
			}else{
			$response = $this->text_response(403, trim($ini["error_page_403"])); 	
			}
        }
        else
        {        
    ...
    {   
        ...
        {        
            ...
            else if ($request->method == 'GET')
            {
                ...
                {        
                    ...
                    {
						$ini = parse_ini_file("webz.ini");
		                if(!isset($ini["error_page_416"])){
                        $response = $this->text_response(416, '<center><h1>416</h1><p>Requested Range not satisfiable</p><hr><sub>WebZHTTPServer/1.0</sub></center>');
						}else{
						$response = $this->text_response(416, trim($ini["error_page_416"]));	
						}
            ...
            {
				$ini = parse_ini_file("webz.ini");
		        if(!isset($ini["error_page_405"])){
                return $this->text_response(405, "<center><h1>405</h1><p>Invalid HTTP method</p><hr><sub>WebZHTTPServer/1.0</sub></center>");
				}
				return $this->text_response(405, trim($ini["error_page_405"]));
            }
        ...
        else if (is_dir($local_path))
        {
			...
			$ini = parse_ini_file("webz.ini");
			$allow_dir_listening = strtolower(trim($ini["allow_dir_listening"]));
			if($allow_dir_listening == "yes"){
					...
					foreach($files as $k=>$v){
						$buffa .= "<br><a href='".$v."'>".$v."</a>";
					}
					$buffa .= "<hr><sub>WebZHTTPServer/1.0</sub></center>";
				    return $this->text_response(200, $buffa);
			}else{
				$ini = parse_ini_file("webz.ini");
		        if(!isset($ini["error_page_403"])){
				return $this->text_response(403, "<center><h1>403</h1><p>Directory listing not allowed</p><hr><sub>WebZHTTPServer/1.0</sub></center>");
				}
				return $this->text_response(403, trim($ini["error_page_403"]));
			}
        }
        else
        {
			$ini = parse_ini_file("webz.ini");
		    if(!isset($ini["error_page_404"])){
				return $this->text_response(404, "<center><h1>404</h1><p>File not found</p><hr><sub>WebZHTTPServer/1.0</sub></center>");
			}
			return $this->text_response(404, trim($ini["error_page_404"]));
        }    
    }        
